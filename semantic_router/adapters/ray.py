...
class RayServeAdapter(ModelAdapter):
    async def _scrape(self) -> dict[str, float]:
        ...
        async with httpx.AsyncClient(timeout=2.0) as cl:
            r = await cl.get(f"{self.base_url}/metrics"); r.raise_for_status()
        cv, wv = [], []
        for line in r.text.splitlines():
            ...
            if "gpu_cache_usage_perc" in p[0]: cv.append(v)
            elif "num_requests_waiting" in p[0]: wv.append(v)
        m = {"kv_cache_max": max(cv, default=0.0), "waiting_total": sum(wv)}
        _CACHE[self.base_url] = (now, m); return m

    async def get_load(self) -> float:
        try:
            m = await self._scrape()
            return max(m["kv_cache_max"], min(m["waiting_total"] / max(self.max_concurrent_requests,1), 1.0))
        except: return 0.5
