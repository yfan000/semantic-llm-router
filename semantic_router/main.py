...
async def chat_completions(request: Request) -> JSONResponse:
    ...
    bids = await collect_bids(adapters, BidRequest(
    ...
    passthrough = {k: v for k, v in body.items() if k not in ("model", "messages", "extra_body")}
    response_dict, headers = await dispatch(
        adapter=winning_adapter, winning_bid=winning_bid, messages=messages,
        domain=meta.domain, complexity=meta.complexity, user_id=sla.user_id,
...
async def register_model(req: RegisterRequest) -> dict:
    ...
    registry.register(ModelConfig(adapter=adapter, domains=req.domains, min_accuracy_capability=capability))
