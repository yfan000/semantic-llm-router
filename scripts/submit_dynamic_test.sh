...
#
#   Phase 1 — Cold start:  qwen-7b + deepseek-r1-7b + qwen3-coder-30b (node1) + llama4-scout (node2)
#   Phase 2 — Warm-up:     100 easy/medium requests at low concurrency (seed models handle)
#   Phase 3 — Code burst:  300 code:hard/medium at high concurrency -> triggers upgrades
...
#
# Key assertions:
#   [PASS] Seed models start (qwen-7b, deepseek-r1-7b, qwen3-coder-30b, llama4-scout)
...
N_BURST=${N_BURST:-300}
CONCURRENCY_WARMUP=${CONCURRENCY_WARMUP:-20}
CONCURRENCY_BURST=${CONCURRENCY_BURST:-80}
...
echo ""
echo "[2] Starting seed models in DYNAMIC mode: qwen-7b + deepseek-r1-7b + qwen3-coder-30b..."
...
echo ""
echo "[3] Waiting for seed models (qwen-7b + deepseek-r1-7b + qwen3-coder-30b + llama4-scout)..."
for i in \$(seq 1 80); do
    ...
    [ "\$N" -ge 4 ] && echo "  Seed models ready." && break
    sleep 15
done
...
echo ""
echo "--- Models after warm-up (should still be 4) ---"
list_models
...
SPINUP_COUNT=\$(grep "SPIN UP" ~/vllm_ogs/prov_dyntest_node1.log 2>/dev/null \
