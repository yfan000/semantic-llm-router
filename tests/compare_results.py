"""
compare_results.py -- Side-by-side comparison of semantic router vs baseline.
...
def summarise(rows: list[dict], electricity_price: float) -> dict:
    ...
    for r in ok:
        ...
        complexity_counts[r["complexity"]][m] = complexity_counts[r["complexity"]].get(m, 0) + 1
    ...
    slo_by_domain: dict[str, tuple[int, int]] = {}
...
def compare(router_path: str, baseline_path: str, electricity_price: float = 0.10) -> None:
    ...
    for cplx in ["easy", "medium", "hard"]:
        ...
        for m in all_models:
            rc = r_dist.get(m, 0)
            bc = b_dist.get(m, 0)
            print(f"    {m:<22} Router {100*rc//max(r_total,1):3}% ({rc:4})   Baseline {100*bc//max(b_total,1):3}% ({bc:4})")
    ...
    print(f"  {'─'*68}")
    row("TOTAL", f"${total_r:.6f}", f"${total_b:.6f}", pct_change(total_b, total_r), total_r < total_b)
    if total_b > 0 and total_r < total_b:
        print(f"\n  Semantic router saved ${total_b-total_r:.6f} ({(total_b-total_r)/total_b*100:.1f}%) vs round-robin")
    elif total_r > total_b:
        print(f"\n  Semantic router cost ${total_r-total_b:.6f} more ({(total_r-total_b)/total_b*100:.1f}%) than round-robin")
    ...
    if R["slo_total"] > 0 or B["slo_total"] > 0:
        ...
        row("Overall",
            ...
            pct_change(b_rate, r_rate), r_rate > b_rate)
    ...
    # -- Retry analysis (router only) -----------------------------------------
    router_ok  = [r for r in router_rows if r.get("status") == "200"]
    retry_rows = [r for r in router_ok if r.get("retries", "") != ""]
    if retry_rows:
        retries  = [int(r["retries"]) for r in retry_rows]
        n_total  = len(retries)
        n_zero   = sum(1 for x in retries if x == 0)
        n_one    = sum(1 for x in retries if x == 1)
        n_two    = sum(1 for x in retries if x == 2)
        n_three  = sum(1 for x in retries if x >= 3)
        avg_r    = sum(retries) / len(retries)
        gt_rows    = [r for r in retry_rows if r.get("gt_scored") == "true"]
        gt_correct = [r for r in gt_rows    if r.get("gt_correct") == "true"]
        gt_wrong   = [r for r in gt_rows    if r.get("gt_correct") == "false"]

        print(f"\n{'='*W}")
        print(f"  RETRY ANALYSIS  (router only — baseline has no retries)")
        print(f"{'='*W}")
        print(f"  Total requests with retry data: {n_total}")
        print(f"")
        print(f"  Retry distribution:")
        print(f"    0 retries (1st model correct):  {n_zero:4}  ({100*n_zero//n_total:3}%)  resolution rate = {100*n_zero/n_total:.1f}%")
        print(f"    1 retry  (2nd model correct):   {n_one:4}  ({100*n_one//n_total:3}%)")
        print(f"    2 retries:                       {n_two:4}  ({100*n_two//n_total:3}%)")
        print(f"    3+ retries:                      {n_three:4}  ({100*n_three//n_total:3}%)")
        print(f"")
        print(f"  Avg retries per request:          {avg_r:.3f}")
        print(f"  Resolution rate (<=1 retry):      {100*(n_zero+n_one)/n_total:.1f}%  ({n_zero+n_one}/{n_total})")

        if gt_rows:
            print(f"")
            print(f"  Ground-truth scored requests:    {len(gt_rows)}")
            print(f"    Correct on final attempt:       {len(gt_correct)}  ({100*len(gt_correct)//len(gt_rows)}%)")
            print(f"    Wrong on all attempts:          {len(gt_wrong)}  ({100*len(gt_wrong)//len(gt_rows)}%)")

        domains = sorted({r["domain"] for r in retry_rows})
        if len(domains) > 1:
            print(f"")
            print(f"  Avg retries by domain:")
            for d in domains:
                dr  = [int(r["retries"]) for r in retry_rows if r["domain"] == d]
                if dr:
                    res = sum(1 for x in dr if x == 0)
                    print(f"    {d:<14} avg={sum(dr)/len(dr):.2f}  "
                          f"0-retry rate={100*res//len(dr)}%  (n={len(dr)})")

    # -- Mode comparison -------------------------------------------------------
    modes_present = sorted({r.get("mode","") for r in router_ok
    ...
    if len(modes_present) > 1:
        ...
        print(f"  Round-robin baseline: wall P50={bw_p50:.0f}ms  "
              f"avg_cost=${bc_avg:.8f}  avg_energy={be_avg:.3f}J  accuracy={b_acc_str}")
        ...
        for mode in modes_present:
            ...
            print(f"    Latency  P50={w_p50:.0f}ms  mean={w_mean:.0f}ms    vs baseline {bw_p50:.0f}ms  {pct_change(bw_p50, w_p50)}")
            print(f"    Cost     avg=${c_avg:.8f}  vs baseline ${bc_avg:.8f}  {pct_change(bc_avg, c_avg)}")
            print(f"    Energy   avg={e_avg:.3f}J  vs baseline {be_avg:.3f}J  {pct_change(be_avg, e_avg)}")
            if m_acc is not None:
                acc_str = f"{m_acc*100:.1f}%  (scored {m_acc_n}/{m_acc_total})"
                b_ref   = b_acc if b_acc is not None else 0.0
                print(f"    Accuracy avg={acc_str}  vs baseline {b_acc_str}  {pct_change(b_ref, m_acc)}")
...
def main() -> None:
    ...
    parser.add_argument("--baseline",          required=True, help="Round-robin or single-model CSV")
