...
QUERIES: dict[tuple[str, str], list[str]] = {
    ...
    ("math", "medium"): [
        ...
        "Find all values of x where sin(x) = 0.5 in [0, 2pi].",
    ...
    ("reasoning", "hard"): [
        ...
        "How does Goedel's incompleteness theorem limit the foundations of mathematics?",
...
async def run(router_url: str, n_requests: int, concurrency: int, output: str,
    ...
    fieldnames = [
        ...
        "charged_usd", "energy_j", "load", "wall_ms", "start_epoch",
    ...
    rate_str = f"{rate:.1f} req/s" if rate else f"closed-loop (concurrency={concurrency})"
...
def main() -> None:
    ...
    parser.add_argument("--rate",        type=float, default=None,
                        help="Target arrival rate in requests/second (open-loop). "
                             ...
                             "Example: --rate 10 sends ~10 req/s regardless of model speed. "
    ...
    if args.no_retry:
        _NO_RETRY = True
        print(f"  [No-Retry] Cascade retry disabled -- single-shot mode")
