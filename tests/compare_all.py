...
def print_summary(systems: list[tuple[str, dict]], ref_name: str | None = None) -> None:
    """Print ranked summary table (all systems, sorted by accuracy desc)."""
    ...
    print(f"\n  {'System':<24} {'Requests':>8} {'Accuracy':>9} {hdr_vs:>9}"
          f"  {'Lat P50':>8}  {'Lat P95':>8}  {'Cost/req':>11}")
    print(f"  {'-'*(W-2)}")

    for name, stats in ranked:
        vs = _pp(stats["accuracy"], ref["accuracy"]) if (ref and name != ref_name) else (
             "  [ref] " if (ref and name == ref_name) else "")
        print(f"  {name:<24} {stats['n']:>8} {_pct(stats['accuracy']):>9} {vs:>9}"
              f"  {_ms(stats['lat_p50']):>8}  {_ms(stats['lat_p95']):>8}  {_usd(stats['cost_mean']):>11}")
...
def save_csv(systems: list[tuple[str, dict]], output: str) -> None:
    ...
    for name, _ in systems:
        ...
        fields += [f"{s}_n", f"{s}_accuracy_pct",
                   f"{s}_lat_p50_ms", f"{s}_lat_p95_ms", f"{s}_cost_mean_usd"]

    with open(output, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields, extrasaction="ignore")
        w.writeheader()

        def _row(cat_label: str, domain: str, complexity: str,
                 getter) -> dict:
