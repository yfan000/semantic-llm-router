...
MODEL_CATALOG: dict[str, ModelSpec] = {
    ...
    "qwen3-coder-30b": ModelSpec(
        model_id="qwen3-coder-30b", model_name="Qwen/Qwen3-Coder-30B-A3B",
        ...
        expected_tokens_per_sec=2500.0,
        max_model_len=8192,
    ...
    "gemma-3-27b": ModelSpec(
        ...
        expected_tokens_per_sec=1000.0,
        max_model_len=4096,
        extra_vllm_args=["--trust-remote-code"],
    ),
...
class DynamicProvisioner:
    ...
    def _should_spin_up_for_ttca(
        ...
        if improvement < TTCA_MIN_IMPROVEMENT:
            log.info("  TTCA gate: improvement=%.1f%% < %.0f%% min -- skip spin-up",
                     improvement * 100, TTCA_MIN_IMPROVEMENT * 100)
            return False
        log.info("  TTCA gate: improvement=%.1f%% >= %.0f%% min -- spin up",
                 improvement * 100, TTCA_MIN_IMPROVEMENT * 100)
        return True
    ...
    async def _try_quality_upgrade(
        ...
        if better_running:
            ...
            log.warning("  Quality upgrade [Case 2 already running]: %s acc=%.3f > %.3f for %s:%s",
        ...
        log.warning("  Quality upgrade [Case 3 ceiling]: no model beats %.3f for %s:%s",
    ...
    async def _try_scale_out(
        ...
        accuracy_is_high = (current_acc is not None and current_acc >= HIGH_ACCURACY_THRESHOLD)
        ...
        if accuracy_is_high:
            ...
            if candidates:
                ...
                log.info("  Strategy A downgrade: %s (%d GPUs, acc=%.3f) for %s",
                         best.model_id, best.gpus_needed, best_acc, domain)
    ...
    async def run(
        ...
        if static:
            log.info("Mode: STATIC -- auto-scaling disabled")
