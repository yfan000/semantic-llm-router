...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrrrrr}
    ...
    \midrule
    Static  & \result{97.1\%} & \result{\todo{X\,s}} & \result{\todo{X\,s}}
            & \result{\todo{X.X}} & \result{\todo{\$X}} & \result{\todo{X}\,h} & \result{84.9\%} \\
    Dynamic & \result{89.2\%} & \result{\todo{X\,s}} & \result{\todo{X\,s}}
            & \result{\todo{X.X}} & \result{\todo{\$X}} & \result{\todo{X}\,h} & \result{82.5\%} \\
    \midrule
    $\Delta$ & $-7.9$pp & \todo{$\pm$X\%} & \todo{$\pm$X\%}
             & \todo{$\pm$X.X} & \result{\todo{X\%}} & \result{\todo{$-$X\%}} & $-2.4$pp \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\ttca\ mean of \result{\todo{X\,s}} and P50 latency of \result{\todo{X\,s}} exceed any sub-2\,s target.
