...
\begin{table}[t]
  ...
  \begin{tabular}{lrrr|r}
    ...
    \midrule
    Factual   & \todo{X\,(X\%)} & \todo{X\,(X\%)} & \todo{X\,(X\%)} & 65\,(22\%) \\
    Math      & \todo{X\,(X\%)} & \todo{X\,(X\%)} & \todo{X\,(X\%)} & 73\,(24\%) \\
    Code      & \todo{X\,(X\%)} & \todo{X\,(X\%)} & \todo{X\,(X\%)} & 117\,(39\%) \\
    Reasoning & \todo{X\,(X\%)} & \todo{X\,(X\%)} & \todo{X\,(X\%)} & 45\,(15\%) \\
    \midrule
    \textbf{Total} & 116 & 85 & 99 & \textbf{300} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrrrrr}
    ...
    CARROT~\cite{somerstep2025carrot} ($\mu{=}0.3$) & \todo{X\%} & \todo{Xs} & \todo{Xs} & 1.0 & \$0.000111 & \todo{X\,h} & \todo{X\%} \\
    OmniRouter~\cite{mei2025omnirouter} ($\alpha{=}0.75$) & \todo{X\%} & \todo{Xs} & \todo{Xs} & 1.0 & \$0.000133 & \todo{X\,h} & \todo{X\%} \\
    Tier-Opt-Acc.\ (oracle)                        & \todo{X\%} & \todo{Xs} & \todo{Xs} & 1.0 & \$0.000142 & \todo{X\,h} & \todo{X\%} \\
    Tier-Opt-Cost (oracle)                         & \todo{X\%} & \todo{Xs} & \todo{Xs} & 1.0 & \$0.0000318 & \todo{X\,h} & \todo{X\%} \\
    \midrule
    \sysname\ (static)                             & \result{97.1\%} & \result{7.1\,s} & \result{17.1\,s} & \result{\todo{X.X}} & \result{\$0.000101} & \result{\todo{X}\,h} & \result{2\%} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[t]
  ...
  \begin{tabular}{llrrrrrrr}
    ...
    \multicolumn{2}{l}{\textbf{Overall}}
      & \todo{X\,/\,\$X} & \todo{X\,/\,\$X} & \todo{X\,/\,\$X} & \todo{X\,/\,\$X} & \todo{X\,/\,\$X} & \todo{X\,/\,\$X} & \result{97.1\%\,/\,\$0.000101} \\
    \bottomrule
  \end{tabular}}
\end{table}
...
\begin{table}[t]
  ...
  \caption{%
    ...
    Avg.\ att.\ = average model calls per request (retries included).%
  }
  \label{tab:dynamic}
  \small
  \setlength{\tabcolsep}{4pt}
  ...
  \begin{tabular}{lrrrrrrr}
    \toprule
    Mode & Accuracy & \ttca\ mean & \ttca\ P95 & Avg.\ att. & Cost/req & GPU-hours & SLO viol. \\
    \midrule
    Static  & \result{97.1\%} & \result{7.1\,s} & \result{17.1\,s}
            & \result{\todo{X.X}} & \result{\$0.000101} & \result{4.7\,h} & \result{2\%} \\
    Dynamic & \result{89.2\%} & \result{7.7\,s} & \result{17.0\,s}
            & \result{\todo{X.X}} & \result{\$0.000144} & \result{3.1\,h} & \result{4\%} \\
    \midrule
    $\Delta$ & $-7.9$pp & $+8\%$ & $\approx$0\%
             & \todo{$\pm$X.X} & $+43\%$ & $-34\%$ & $+2$\,pp \\
    \bottomrule
  \end{tabular}}
\end{table}

\paragraph{\ttca\ by domain and complexity.}
Table~\ref{tab:ttca_breakdown} breaks down \ttca\ mean per
(domain, complexity) cell for both provisioning modes.
Code and reasoning cells show the largest delta, as the seed-only
dynamic fleet lacks the specialist models needed to resolve hard
queries quickly.

\begin{table}[t]
  \centering
  \caption{%
    \ttca\ mean (seconds) by domain and complexity for static and dynamic
    provisioning (correct answers only, 300-request workload).
    $\Delta = \text{Dynamic} - \text{Static}$; positive~$=$ dynamic is slower.%
  }
  \label{tab:ttca_breakdown}
  \small
  \begin{tabular}{llrrr}
    \toprule
    Domain & Cmplx & Static (s) & Dynamic (s) & $\Delta$ (s) \\
    \midrule
    \multirow{3}{*}{Factual}
      & Easy   & \todo{X.X} & \todo{X.X} & \todo{$\pm$X.X} \\
      & Medium & \todo{X.X} & \todo{X.X} & \todo{$\pm$X.X} \\
      & Hard   & \todo{X.X} & \todo{X.X} & \todo{$\pm$X.X} \\
    \midrule
    \multirow{3}{*}{Math}
      & Easy   & \todo{X.X} & \todo{X.X} & \todo{$\pm$X.X} \\
      & Medium & \todo{X.X} & \todo{X.X} & \todo{$\pm$X.X} \\
    ...
    \midrule
    \multicolumn{2}{l}{\textbf{Overall}}
      & \result{7.1} & \result{7.7} & \result{$+0.6$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
experiment (including \result{10} min of model loading),
accumulating \result{4.7\,h} of GPU-core-hours on Node~1.
Dynamic mode starts with \result{4} GPUs (seed fleet); in this run
no additional models were provisioned beyond the seeds
(the provisioner never triggered a scale-out event), consuming
\result{3.1\,h} — a \result{34\%} reduction.
This saving comes entirely from not loading the three heavier models
(Gemma-3-27B, DeepSeek-R1-14B, Llama-4-Scout) that static mode
keeps resident throughout.
...
Both modes achieve low overall SLO violation rates:
\result{2\%} for static (\result{8}/294 requests) and \result{4\%} for dynamic
(\result{12}/300 requests).
\ttca\ mean is \result{7.1\,s} for static and \result{7.7\,s} for dynamic,
with P50 latency of \result{6.0\,s} and \result{6.6\,s} respectively —
well within the 30\,s per-request timeout used by the evaluation harness.
The slight increase from 2\% to 4\% under dynamic provisioning
reflects the seed-only fleet: without the larger specialists loaded,
hard code and reasoning requests take longer, causing a few additional violations.
